...
import { ArrowRight, ShieldCheck, HeartHandshake, Target, Sparkles } from "lucide-react"
...
const values = [
  {
    ...
    description: "Every job is carried out to current safety standards — no shortcuts, no compromises.",
  },
  {
    icon: HeartHandshake,
    title: "Honest & transparent",
    description: "Clear pricing and straight answers, so you always know what's happening and why.",
  },
  {
    icon: Target,
    title: "Get it right first time",
    description: "We take the time to diagnose properly, so repairs last and installations perform.",
  },
  {
    icon: Sparkles,
    title: "Respect for your property",
    description: "Tidy work, protected floors and surfaces, and a clean site when we're done.",
  },
]

const team = [
  { role: "Lead Gas & Heating Engineer", focus: "Gas installations, boiler servicing & safety checks", image: photos.workerHardhat1 },
  { role: "Senior Plumbing Engineer", focus: "Bathroom fit-outs, leak detection & pipework", image: photos.workerPortrait1 },
  { role: "Electrical Engineer", focus: "Installations, fault-finding & repairs", image: photos.workerSafetyHelmet },
  { role: "Operations & Scheduling", focus: "Bookings, quotes & customer support", image: photos.workerOrangeHardhat },
]
        ...
        <div className="container-custom grid grid-cols-1 lg:grid-cols-2 gap-14 items-center">
          ...
          <div>
            ...
            <div className="mt-6 space-y-4 text-muted-foreground leading-relaxed">
              <p>
                Gas Safety and Plumbing Engineering provides professional gas, heating, plumbing, electrical
                and general maintenance services for domestic and commercial properties across Enfield and
                London.
              </p>
              <p>
                From boiler installations, servicing and repairs to central heating systems, hot water
                solutions, plumbing repairs, electrical works and property maintenance, our team delivers
                reliable, safe and efficient solutions tailored to your needs.
              </p>
              <p>
                Whether it's a routine service, a planned installation, or an emergency repair, we keep your
                property safe, warm and running efficiently — with the same attention to detail every time.
              </p>
            </div>
            <Reveal delay={0.2}>
              <Link
            ...
            {values.map((value, i) => (
              <Reveal key={value.title} delay={i * 0.08}>
                <div className="h-full rounded-2xl border border-border bg-white p-6">
                  ...
                  <h3 className="mt-4 font-display font-bold text-brand-blue-950">{value.title}</h3>
            ...
            {team.map((member, i) => (
              <Reveal key={member.role} delay={i * 0.08}>
                <div className="rounded-2xl border border-border bg-white overflow-hidden card-hover">
                  ...
                  <div className="p-5">
                    <h3 className="font-display font-bold text-brand-blue-950 text-sm">{member.role}</h3>
                    <p className="mt-1 text-xs text-muted-foreground leading-relaxed">{member.focus}</p>
                  </div>
                </div>
              </Reveal>
            ))}
